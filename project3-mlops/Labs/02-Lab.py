...
with mlflow.start_run(run_name= "RF-Grid-Search") as run:
  ...
  # FILL_IN
    ...
    print("Inside MLflow Run with id {}".format(runID),"artifactURI {}".format(artifactURI))
...
with mlflow.start_run(run_name= "RF-Grid-Search_2") as run:
  # Create predictions of X_test using best model
  # FILL_IN
    rf=best_rf
    rf.fit(X_train,y_train)
  ...
  # Log model with name
  # FILL_IN
    mlflow.sklearn.log_model(rf, "grid-random-forest-model_2")
...
client.search_runs(run.info.experiment_id)
